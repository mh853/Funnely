...
-- 전화번호 검색 (NULL 값 허용)
CREATE INDEX IF NOT EXISTS idx_users_phone_search ON users USING gin(to_tsvector('simple', COALESCE(phone, '')));
...
-- 성능 확인을 위한 코멘트
COMMENT ON INDEX idx_users_last_login IS 'Phase 4: 마지막 로그인 정렬 성능 향상';
...
COMMENT ON INDEX idx_users_phone_search IS 'Phase 4: 전화번호 검색 성능 향상';
