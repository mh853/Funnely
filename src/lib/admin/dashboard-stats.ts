// 어드민 대시보드 통계 조회 — API 라우트(GET)와 서버 컴포넌트(admin/page.tsx)가
// 공유하는 데이터 조회 로직. route.ts 밖에 둬야 하는 이유는 Next.js App Router가
// route.ts의 export를 HTTP 메서드/설정 키로만 검증하기 때문에, 다른 export를
