...
import { KeyIcon, TagIcon, TableCellsIcon, Cog6ToothIcon, BuildingOffice2Icon, UserCircleIcon, CreditCardIcon, CurrencyDollarIcon, UsersIcon, ChartBarIcon, BellIcon, GlobeAltIcon, BookOpenIcon, TrashIcon } from '@heroicons/react/24/outline'
  ...
  return (
    <div className="px-4 space-y-6">
      ...
      <div className="grid gap-3 grid-cols-[repeat(auto-fill,minmax(13rem,1fr))]">
        ...
        <Link
          href={isMetaEnabledForCompany(userProfile.company_id) ? '/dashboard/ad-performance' : '/dashboard/settings/api-credentials'}
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-medium text-gray-900 truncate">{isMetaEnabledForCompany(userProfile.company_id) ? '광고 계정 연동' : '광고 플랫폼 API'}</h3>
            <p className="text-xs text-gray-500 truncate">{isMetaEnabledForCompany(userProfile.company_id) ? 'Meta 광고 성과 조회' : 'Meta, Kakao, Google Ads'}</p>
          </div>
        ...
        {isAdmin && (
          <Link
            href="/dashboard/settings/lead-deletion-logs"
            className="group flex items-center gap-3 bg-white border border-gray-200 hover:border-red-300 hover:bg-red-50 rounded-xl px-4 py-3 transition-all shadow-sm hover:shadow"
          >
            <div className="flex-shrink-0 bg-red-100 rounded-lg p-2 group-hover:bg-red-200 transition-colors">
              <TrashIcon className="h-5 w-5 text-red-600" />
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-medium text-gray-900 truncate">DB 삭제 이력</h3>
              <p className="text-xs text-gray-500 truncate">DB현황 삭제 계정/건수 조회</p>
            </div>
      ...
      <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-gray-100">
        ...
        <div className="p-4">
          <div className={`grid gap-3 grid-cols-1 ${canEdit ? 'sm:grid-cols-3' : 'sm:grid-cols-1'}`}>
            {/* 구독 관리/결제 내역 - company_subscriptions RLS(admin/owner만 허용, manager
                제외)와 동일 범위로 제한. 카드 등록/결제전환 API 자체도 이 범위로 막혀있어
                링크를 보여줘도 어차피 실패하므로 애초에 노출하지 않는다. */}
            {canEdit && (
              <>
                <Link
