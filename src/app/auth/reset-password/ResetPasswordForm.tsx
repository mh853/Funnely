// 비밀번호 재설정 폼 UI 및 제출 로직을 담당하는 클라이언트 컴포넌트
'use client'
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 px-4">
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          {/* 서버가 발급한 pwreset_verified 쿠키가 없으면(즉, 방금 복구 링크를 거쳐오지 않았다면)
              실제 로그인 세션이 있더라도 무조건 만료 화면만 보여준다. 세션 존재 여부만으로
              "복구 링크를 거쳐왔다"고 판단하면 활성 세션만 확보해도 현재 비밀번호 확인 없이
              비밀번호를 바꿀 수 있는 계정 탈취 경로가 열린다. */}
          {!verified || !hasSession ? (
