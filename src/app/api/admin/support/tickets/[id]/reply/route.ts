...
// 고객에게 답변 알림 - 인앱 알림 + 즉시 이메일. 둘 다 실패해도 답변 생성/수정 자체는
// 이미 성공했으므로 응답을 막지 않는다(best-effort, non-critical). 답변 "수정"(PATCH)이
// POST와 동일한 알림을 안 보내던 것을 71차 QA에서 발견해 공용 함수로 뽑아 재사용한다.
async function notifyTicketReply(
  ...
  const dashboardUrl = process.env.NEXT_PUBLIC_DOMAIN
    ? `${process.env.NEXT_PUBLIC_DOMAIN.replace(/\/$/, '')}/dashboard/support/${ticketId}`
    : `https://funnely.co.kr/dashboard/support/${ticketId}`
...
export async function POST(
  ...
  try {
    ...
    await notifyTicketReply(supabase, params.id, reply_message.trim())
