...
**현재 Phase**: Phase 1 완료 ✅ / Phase 2 (어드민 대시보드) 준비 중
...
- [ ] `src/app/admin/AdminNav.tsx` - 네비게이션
- [ ] `src/app/admin/page.tsx` - 대시보드 메인

#### 2.2 대시보드 통계
- [ ] `/api/admin/stats` API 엔드포인트
- [ ] `StatsCard.tsx`, `RecentCompanies.tsx`, `SystemAlerts.tsx` 컴포넌트
- [ ] 실시간 데이터 갱신 (SWR/React Query)
...
**현재 작업**: Phase 2.1 - 어드민 레이아웃 및 네비게이션

**순서**:
1. ✅ Phase 1 완료 (DB 마이그레이션, 권한 시스템, 미들웨어)
2. 🔜 어드민 레이아웃 생성 (layout.tsx, AdminNav.tsx)
3. 대시보드 메인 페이지 (page.tsx)
4. 통계 API 엔드포인트
5. 통계 카드 컴포넌트
